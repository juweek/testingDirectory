...
function boxplotStats(data, valueof) {
  ...
  const fiveNums = [0.0, 0.25, 0.5, 0.75, 1.0].map((d) => d3Array.quantile(values, d));
  ...
  const whiskers = [
    { start: d3Array.min(values.filter((d) => fences[1].start <= d)), end: fiveNums[1] },
    { start: d3Array.max(values.filter((d) => fences[3].end >= d)), end: fiveNums[3] },
  ];
...
exports.boxplotStats = boxplotStats;
exports.boxplotSymbolDot = boxplotSymbolDot;
exports.boxplotSymbolTick = boxplotSymbolTick;
